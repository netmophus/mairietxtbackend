...
//   try {
//     const { taxpayerId } = req.params;
//     // Le payload doit maintenant contenir { taxes: [...], surfaces: { taxId1: surface1, taxId2: surface2, ... } }
...
//     // Récupérer les taxes spécifiées
//     const existingTaxes = await Tax.find({ _id: { $in: taxes } });
...
//     // Créer des entrées dans TaxpayerTax pour chaque taxe sélectionnée
//     for (const tax of existingTaxes) {
//       // Vérifier s'il existe déjà une entrée pour cette taxe et ce contribuable
//       const existingEntry = await TaxpayerTax.findOne({
//         taxpayer: taxpayerId,
//         tax: tax._id,
//       });
...
//         if (tax.isVariable) {
//           // Pour la taxe d'occupation, on calcule le montant total à partir de la surface
//           if (tax.name === "Taxe d'occupation du domaine publique") {
//             // On attend que le payload contienne un objet "surfaces" avec une valeur pour cette taxe
...
//             }
//             totalAmount = surface * tax.supportRates.default;
//             remainingAmount = totalAmount;
//             console.log(`Taxe d'occupation : surface = ${surface} m², taux = ${tax.supportRates.default} FCFA/m², totalAmount = ${totalAmount} FCFA`);
//           } else {
//             // Pour d'autres taxes variables, vous pouvez définir la logique appropriée.
//             totalAmount = 0;
//             remainingAmount = 0;
//             console.log(`Taxe variable (non occupation) détectée pour ${tax.name} : totalAmount initialisé à 0.`);
//           }
//         } else {
//           // Pour les taxes fixes, on utilise tax.amount
...
//           totalAmount,
//           remainingAmount,
//           dueDate: tax.dueDate, // On reprend la date d'échéance définie dans la taxe
//           status: 'pending',
//           // On peut aussi enregistrer la surface ici si nécessaire
//           surface: tax.isVariable && tax.name === "Taxe d'occupation du domaine publique" ? surfaces[tax._id] : undefined,
//         });
...
//     // Mettre à jour le contribuable avec les nouvelles taxes associées
...
module.exports = { createTaxpayer, getTaxpayers, getTaxpayersWithTaxes, associateTaxesToTaxpayer, getTaxpayerByPhone };
